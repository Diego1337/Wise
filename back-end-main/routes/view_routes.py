    ...
    query = text("""
        ...
        FROM (
            SELECT 
                ...
                mp.nome_mp AS nome_materia_prima, -- Use 'nome' conforme o seu diagrama (image_68a876.png)
                -- Seu diagrama não mostra 'uni_medida' na MP, mas vamos assumir que existe
                mp.uni_medida AS unidade_medida, 
                f.id_fornec AS id_fornecedor,
                f.nome_fornec AS nome_fornecedor, -- Use 'nome_fantasia' conforme o seu diagrama
                p.preco_mp AS preco,
                p.data_preco AS data_cotacao,
                -- Atribui um número de linha a cada preço por id_mp, ordenado pela data
                ROW_NUMBER() OVER (
                    PARTITION BY mp.id_mp 
                    ORDER BY p.data_preco DESC, p.id_preco DESC
                ) AS rn
            FROM tbl_preco AS p
